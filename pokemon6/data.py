...
def coord_2_loc_obs(coord, label, ll = L):
  multi = 1.0 if label[0] == 1.0 else -1.0
  ret = np.zeros([ll, ll, 1])
  for i in range(ll):
    for j in range(ll):
      grid_coord_i = i + 0.5
      grid_coord_j = j + 0.5
      ret[i][j][0] = np.exp(-dist((grid_coord_i, grid_coord_j), coord)) * multi
  return ret
...
def gen_data(n_batch = n_batch, K=K):
  ...
  for bb in range(n_batch):
    ...
    _last_weights = [0.0 for _ in range(K)]
    _last_weights[k-1] = 1.0
    last_weights.append(_last_weights)
  ...
  return k_locs, \
         ...
         np.array(last_weights, np.float32), \
