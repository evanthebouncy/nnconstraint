...
def gen_data(n_batch = n_batch, K=K):
  ...
  # LIST of length K (1 for each input)
  # each element of shape [batch x 2] (2 classes, T or F)
  k_TFs = [[] for i in range(K)]
  ...
  for bb in range(n_batch):
    ...
    for _ in range(k):
      ...
      obs_x_loc = coord_2_loc(obs_x)
      b_k_locs.append(obs_x_loc)
      b_k_TFs.append(obs_lab)

    b_k_locs = pad_and_numpy(b_k_locs, K) 
    b_k_TFs = pad_and_numpy(b_k_TFs, K) 
    for kkk in range(K):
      k_locs[kkk].append(b_k_locs[kkk]) 
      k_TFs[kkk].append(b_k_TFs[kkk]) 
